...
namespace NetworkPerspective.Sync.Infrastructure.Google.Tests.Services
{
    public class CalendarClientTests : IClassFixture<GoogleClientFixture>
    {
        ...
        public async Task ShouldReturnInteractionsBasedOnGoogleCalendar()
        {
            ...
            var client = new CalendarClient(Mock.Of<ITasksStatusesCache>(), Options.Create(googleConfig), new ThrottlingRetryHandler(), _googleClientFixture.CredentialProvider, NullLogger<CalendarClient>.Instance);
