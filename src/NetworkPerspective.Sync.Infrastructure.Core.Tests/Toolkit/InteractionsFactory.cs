...
namespace NetworkPerspective.Sync.Infrastructure.Core.Tests.Toolkit
{
    internal static class InteractionsFactory
    {
        ...
        public static Interaction Create()
        {
            ...
            var source = Employee.CreateInternal($"email_source_{_counter}@networkperspective.io", Guid.NewGuid().ToString(), $"email_source_{_counter}_manager@networkperspective.io", new[] { group1, group2 });
            var target = Employee.CreateInternal($"email_target_{_counter}@networkperspective.io", Guid.NewGuid().ToString(), $"email_target_{_counter}_manager@networkperspective.io", new[] { group2 });
