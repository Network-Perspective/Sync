...
public class SyncSchedulerTest
{
    ...
    private readonly IOptions<SyncSchedulerConfig> _defaultOptions = CreateOptions();
    ...
    public class TriggerNow : SyncSchedulerTest
    {
        ...
        public async Task ShouldTriggerWithCorrectConnectorId()
        {
            ...
            // Assert
            SpinWait.SpinUntil(() => TestableJob.ExecutedJobs.Any(x => x == connectorId.ToString()), TimeoutInMs).Should().BeTrue();
        }
    }
    ...
    public class InterruptNow : SyncSchedulerTest
    {
        ...
        public async Task ShouldNotInterfereWithOtherNetworksSync()
        {
            ...
            // Assert
            SpinWait.SpinUntil(() => TestableJob.CompletedJobs.Contains(connectorId_2.ToString()), TimeoutInMs).Should().BeTrue();
        }
    }
    ...
    public class Unschedule : SyncSchedulerTest
    {
        ...
        public async Task ShouldNotInterfereWithOtherConnectorSync()
        {
            ...
            // Assert
            SpinWait.SpinUntil(() => TestableJob.ExecutedJobs.Any(x => x == connectorId_2.ToString()), TimeoutInMs).Should().BeTrue();
        }
    }
    ...
    private static IOptions<SyncSchedulerConfig> CreateOptions(string cronExpression = "0/1 * * * * ?")
        => Options.Create(new SyncSchedulerConfig { CronExpression = cronExpression });
