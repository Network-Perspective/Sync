...
public interface ISyncScheduler
{
    ...
    Task<bool> IsScheduledAsync(Guid neconnectorIdtworkId, CancellationToken stoppingToken = default);
...
internal class SyncScheduler : ISyncScheduler
{
    ...
    public async Task EnsureRemovedAsync(Guid connectorId, CancellationToken stoppingToken = default)
    {
        ...
        if (isDeleted)
            ...
            _logger.LogDebug("Nothing to remove... schedule for Connector '{connectorId}' doesn't exist", connectorId);
    }
    ...
    public async Task ScheduleAsync(Guid connectorId, CancellationToken stoppingToken = default)
    {
        ...
        var nextExecutionTime = (await scheduler.GetTrigger(triggerKey, stoppingToken)).GetNextFireTimeUtc();
    ...
    public async Task UnscheduleAsync(Guid connectorId, CancellationToken stoppingToken = default)
    {
        ...
        var triggers = await scheduler.GetTriggersOfJob(CreateJobKey(connectorId), stoppingToken);
